        ...
        for ity=1:nyear-1 
            ...
            %else
                for itp=1:73  %counter for 
                    ...
                    if itp == 1
                        S(ity,itp)=squeeze(pentad(inipent,ity)-pentclim(itp,statn)); 
                        S2(ity,itp)=pentad(inipent2,ity)-pentclim(itp,statn);
                    else
                        if itk<=73
                            S(ity,itp)=nansum(pentad(inipent:itk,ity)-pentclim(itp,statn),1);
                        else %use next year's data  
                            S0=nansum(pentad(inipent:73,ity)-pentclim(itp,statn),1); %accumulated until the end of previous year
                            S(ity,itp)= S0 + ( nansum(pentad(1:itk-73,ity+1)-pentclim(itp,statn),1) ); %total, considering demise on following year
                         end
                        if itk2<=73  %demise
                            S2(ity,itp)=nansum(pentad(inipent2:itk2,ity)-pentclim(itp,statn),1);
                        else %use next year's data  
                            S02=nansum(pentad(inipent2:73,ity)-pentclim(itp,statn),1); %accumulated until the end of previous year
                            S2(ity,itp)= S02 + ( nansum(pentad(1:itk2-73,ity+1)-pentclim(itp,statn),1) ); %total, considering demise on following year
                        end
                    end
                end
            %end
